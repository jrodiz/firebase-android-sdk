// Copyright 2026 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
//
// You may obtain a copy of the License at
//      http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package com.google.firebase.perf.metrics;

import static com.google.common.truth.Truth.assertThat;
import static org.robolectric.Shadows.shadowOf;

import android.app.ActivityManager;
import android.app.ActivityManager.RunningAppProcessInfo;
import android.content.Context;
import android.os.Process;
import androidx.test.core.app.ApplicationProvider;
import java.util.Collections;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.robolectric.RobolectricTestRunner;
import org.robolectric.annotation.Config;
import org.robolectric.shadows.ShadowActivityManager;

/** Unit tests for {@link ProcessStartCause}. */
@RunWith(RobolectricTestRunner.class)
public class ProcessStartCauseTest {

  private final Context appContext = ApplicationProvider.getApplicationContext();

  /** Seeds {@code getMyMemoryState}'s reply with the desired importance for this process. */
  private void setProcessImportance(int importance) {
    ActivityManager am = (ActivityManager) appContext.getSystemService(Context.ACTIVITY_SERVICE);
    ShadowActivityManager shadow = shadowOf(am);
    RunningAppProcessInfo info = new RunningAppProcessInfo();
    info.pid = Process.myPid();
    info.importance = importance;
    shadow.setProcesses(Collections.singletonList(info));
  }

  @Test
  public void capture_nullContext_returnsUnknown() {
    ProcessStartCause cause = ProcessStartCause.capture(null);

    assertThat(cause.cause).isEqualTo(ProcessStartCause.Cause.UNKNOWN);
    assertThat(cause.reasonName).isEmpty();
    assertThat(cause.startTypeName).isEmpty();
    assertThat(cause.importance).isEqualTo(-1);
  }

  @Test
  @Config(sdk = 33)
  public void capture_preApi34_returnsUnknownButRecordsImportance() {
    setProcessImportance(RunningAppProcessInfo.IMPORTANCE_FOREGROUND);

    ProcessStartCause cause = ProcessStartCause.capture(appContext);

    // Pre-API-34: we never classify (the existing pre-34 logic is correct).
    assertThat(cause.cause).isEqualTo(ProcessStartCause.Cause.UNKNOWN);
    assertThat(cause.apiLevel).isEqualTo(33);
    // importance is captured for telemetry continuity even when we don't classify.
    assertThat(cause.importance).isEqualTo(RunningAppProcessInfo.IMPORTANCE_FOREGROUND);
    // API 35+ fields stay empty.
    assertThat(cause.reasonName).isEmpty();
    assertThat(cause.startTypeName).isEmpty();
  }

  @Test
  @Config(sdk = 34)
  public void capture_api34_foregroundImportance_classifiesForeground() {
    setProcessImportance(RunningAppProcessInfo.IMPORTANCE_FOREGROUND);

    ProcessStartCause cause = ProcessStartCause.capture(appContext);

    assertThat(cause.cause).isEqualTo(ProcessStartCause.Cause.FOREGROUND);
    assertThat(cause.importance).isEqualTo(RunningAppProcessInfo.IMPORTANCE_FOREGROUND);
    assertThat(cause.apiLevel).isEqualTo(34);
    assertThat(cause.reasonName).isEmpty();
    assertThat(cause.startTypeName).isEmpty();
  }

  @Test
  @Config(sdk = 34)
  public void capture_api34_serviceImportance_classifiesUnknown() {
    // A non-FOREGROUND importance means we cannot positively confirm foreground intent
    // on API 34. We deliberately return UNKNOWN rather than BACKGROUND so the caller
    // falls back to the existing heuristic.
    setProcessImportance(RunningAppProcessInfo.IMPORTANCE_SERVICE);

    ProcessStartCause cause = ProcessStartCause.capture(appContext);

    assertThat(cause.cause).isEqualTo(ProcessStartCause.Cause.UNKNOWN);
    assertThat(cause.importance).isEqualTo(RunningAppProcessInfo.IMPORTANCE_SERVICE);
  }

  @Test
  @Config(sdk = 34)
  public void capture_api34_cachedImportance_classifiesUnknown() {
    setProcessImportance(RunningAppProcessInfo.IMPORTANCE_CACHED);

    ProcessStartCause cause = ProcessStartCause.capture(appContext);

    assertThat(cause.cause).isEqualTo(ProcessStartCause.Cause.UNKNOWN);
    assertThat(cause.importance).isEqualTo(RunningAppProcessInfo.IMPORTANCE_CACHED);
  }

  // API 35+ reflective behavior is exercised end-to-end by the experiment app at
  // ~/AndroidStudioProjects/api34-appstart-cause/, run on an API 35 emulator. The repo's
  // Robolectric (4.12) doesn't support @Config(sdk = 35), so we don't unit-test the
  // reflective path here — that would only validate Robolectric's stub, not real OS
  // behavior. See RESULTS.md (API 35 section) for the real-environment data.

  @Test
  public void applicationStartInfoView_holdsValues() {
    ProcessStartCause.ApplicationStartInfoView view =
        new ProcessStartCause.ApplicationStartInfoView(6, 1);

    assertThat(view.reason).isEqualTo(6);
    assertThat(view.startType).isEqualTo(1);
  }

  @Test
  public void constructor_visibleForTesting_preservesAllFields() {
    ProcessStartCause cause =
        new ProcessStartCause(
            ProcessStartCause.Cause.FOREGROUND,
            "LAUNCHER",
            "COLD",
            RunningAppProcessInfo.IMPORTANCE_FOREGROUND,
            35);

    assertThat(cause.cause).isEqualTo(ProcessStartCause.Cause.FOREGROUND);
    assertThat(cause.reasonName).isEqualTo("LAUNCHER");
    assertThat(cause.startTypeName).isEqualTo("COLD");
    assertThat(cause.importance).isEqualTo(RunningAppProcessInfo.IMPORTANCE_FOREGROUND);
    assertThat(cause.apiLevel).isEqualTo(35);
  }
}
